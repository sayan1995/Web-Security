...
<div class="test">
    ...
    <br>
    <h5 class="teal-text" style="position :relative; left: 20px;">Almost there!</h5>
    <br>
    ...
    <div style="position: relative; left: 35%; width: 330px ">
        <form method="POST" action = "<?php $_PHP_SELF ?>">
            <div class="input-field col s6">
              ...
              <label for="sid">Enter the Session ID</label>
            </div>
			
            <div class="input-field">
                <button class="waves-effect waves-light btn blue" type="submit" style="position: relative; left: 15%">Submit</button>
                <a class="btn tooltipped red" data-position="bottom" data-delay="50" style="position: relative; left: 15%" data-tooltip="Cookies">Hint</a>
            </div>
        </form>
	</div>	
<?php
	...
	if (isset($_POST['sid'])) {
			$sid=$_POST['sid'];
			$osid=session_id();
			//echo "<br> Cookie is ".$osid."<br>";
			if ($sid == $osid) {
				//$_SESSION['login_user']=$username; // Initializing Session
				ob_start();
				header('Location: session.php');
				} else {
				$message = "Wrong Session ID. Try Again";
