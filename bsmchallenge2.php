...
<div class="test">
    ...
    <br>
    <h5 class="teal-text" style="position :relative; left: 20px;">Fix the Session ID of the session in order to gain unauthorised access to another user profile</h5><br><br>
	<h6 class="red-text" style="position :relative; left: 30%;">Add any Session ID to the URL you want to send to another user </h6>
	
    <div style="position: relative; left: 35%; width: 330px ">
        <form method="POST" action = "<?php $_PHP_SELF ?>">
            <div class="input-field col s6">
              <input id="sesid" type="text" value="login.php" class="validate" name="sesid">
            </div>
            <div class="input-field">
				...
				<a class="btn tooltipped red" data-position="bottom" data-delay="50" style="position: relative; left: 15%" data-tooltip="Get the name of the Cookie associated with the Session">Hint</a>
            </div>
        </form>
...
<?php
		...
		$sesid=$_POST['sesid'];
		if (strpos($sesid, '?PHPSESSID=') !== false) {
		//$_SESSION['login_user']=$username;
		$cook=substr($sesid,20);
		setcookie("PHPSESSID",$cook,time()+3600,'/');
		ob_start();
		header("Location: ".$sesid); // Redirecting To Other Page
		} else {
